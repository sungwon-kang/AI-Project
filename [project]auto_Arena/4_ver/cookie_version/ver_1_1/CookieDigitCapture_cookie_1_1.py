...
class DigitCapture(QMainWindow):
    ...
    def startThreadFunction(self):
        ...
        self.timer.start(600*ms)
        self.setText("[자동 탐색 시작]\n10분마다 자동으로 마우스가 이동됩니다.\n주의하세요!")
    ...
    def ThreadFunction(self):
        if self.Length>0 and self.Thread_flag==True:
            self.startThreadFunction()
        
        elif self.Thread_flag==False:
    ...
    def run(self):
        ...
        if(len(opponents)>0):
            ...
            self.setText("[상대 발견]")
            self.append(opponents)
            self.append("자동 탐색은 계속 진행됩니다.\n10분 이내에 모든 전투를 끝내세요!")
    ...
    def findOppsUCanwin(self, lstPowerNums,lstThrophyNums):
        ...
        for i in range(n):
            ...
            if(boundThrophy>=Throphy or boundMax>=Power):
                opponents.append((Power,Throphy))
    ...
    def showGuideFunction(self):
        QMessageBox.information(self, '사용법', 
                                ...
                                '  2. (1)을 반복해서 리스트에 추가할 수 있습니다.\n'+
                                '  3. <M>을 눌러 [무료 새로고침]의 좌표를 입력하세요.\n'+
                                '  4. <E> 또는 [시작] 버튼을 눌러 자동 탐색이 시작됩니다.\n'+
                                '  5. <E> 또는 [멈춤] 버튼을 눌러 중단할 수 있습니다.\n\n'+
                                ...
                                ' 전투력 분할 수: 캡쳐한 전투력 이미지 분할 수 설정값\n(기본값 :7)\n'+
                                ' 트로피 분할 수: 캡쳐한 트로피 이미지 분할 수 설정값\n(기본값 :4)\n\n'+
                                
                                
                                '[쿠키런 설정]\n'+
                                ' 상대 트로피 최대치: 탐색할 상대방의 트로피 최대값(기본값 :2900)\n'+
                                ' 상대 전투력 최대치: 탐색할 상대방의 전투력 최대값(기본값 :120만)\n\n'+
                                
                                
                                '[주의 사항]\n'+
                                ' - [시작] 또는 <E>를 눌렀을 때 10분 후에 첫 탐색이 시작됩니다.\n'+
                                ' - 10분 마다 마우스를 강제이동하기 때문에 작업 중에 조심하시길 바랍니다.\n'
                                ' - 알람이 울릴 시 10분 이내에 탐색된 상대와 전투를 끝내세요.\n'
                                )
    ...
    def append(self, msg):
        if type(msg) is list:
            for pair in msg:
                self.te_event.append("{}".format(pair))
        else:
    ...
    def setUI(self):
        ...
        frame_middle.setGeometry(0, 149, self.WindowWidth, 150)
        frame_middle.setFrameShape(QFrame.Box | QFrame.Plain)
        
        frame_bottom = QFrame(self)
        frame_bottom.setGeometry(0, 298, self.WindowWidth, 152)
        ...
        lb_fx1.setGeometry(175, 35, 130, 25)
        self.le_fx1.setGeometry(175, 60, 130, 25)
        
        lb_fx2.setGeometry(335, 35, 130, 25)
        self.le_fx2.setGeometry(335, 60, 130, 25)

        lb_cropPower_n.setGeometry(10, 85, 180, 25)
        self.le_cropPower_n.setGeometry(10, 110, 130, 25)
        
        lb_cropthrophy_n.setGeometry(175, 85, 180, 25)
        self.le_cropthrophy_n.setGeometry(175, 110, 130, 25)
        ...
        lb_EnemyMaxtrophy = QLabel('상대 트로피 최대치', self)
        ...
        lb_EnemyMaxPower = QLabel('상대 전투력 최대치', self)
        ...
        btn_reset.clicked.connect(self.setValueFunction)
